...
import { Box, Center,Icon, Divider, HStack ,VStack,Modal,FormControl,Input,Button,SearchBar } from 'native-base';
    ...
    retrieveData()
      .then(() => {
        axios
          ...
          .get(`http://192.168.1.12:5000/flskills/freelancerSkills/${id}`)
...
useEffect(()=>{ 
  axios.get("http://192.168.1.12:5000/skills/getAll") 
  .then((res)=>{setData(res.data)}) 
  .catch((err)=>{console.log(err);})
},[searched !== ""])
      
const addSkill=(skill_id)=>{  
  axios.post(`http://192.168.1.12:5000/flskills/addSkill`,{fl_id : id , skill_id :skill_id} ) 
  .then((res)=>{console.log("skill added")}) 
  .catch((err)=> console.log(err))
}  
            ...
            <FlatList
             ...
             renderItem={({ item }) => {
               return (   
                ...
                <TouchableOpacity onPress={()=>{addSkill(item.skills_id)}}>
